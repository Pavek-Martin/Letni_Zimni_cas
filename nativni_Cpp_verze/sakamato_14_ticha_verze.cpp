...
int main(int argc, char** argv) {
	// pridano 16.5 2025 maximalizuje se okno z hlasenim v pripade pokud je zmena casu a nebo v pripade chyby
	// ziska handle aktualniho konzoloveho okna
    HWND hWnd = GetConsoleWindow();
    if (hWnd == NULL) {
    cout<<"nelze ziskat handle okna"<<endl;    
    cout<<"program predcasne ukoncen"<<endl;
    system("pause");
    exit(1);
}
    //cout<<hWnd<<"<-- toto je handle aktualniho okna"<<endl;
    
    // minimalizace okna s programem
    //ShowWindow(hWnd, SW_SHOWMINIMIZED);
	/* tento prikaz minimalizuje konzolove okno ve kterem bezi tento program
    v zastupci programu "exe" tohoto programu ve slozce "po spusteni" nastavit, spoustet program v
    minimalizovanem okne, paklize by program v nejakem miste (napr. radek 171) okno maximalizoval
	bude toto nastaveni zastupce programu ignorovano a hlaseni o zmene casu a nebo chybove hlaseni
	se zobrazi v normalni (strednim) a nebo v maximalizovanem konzolovem okne :)
	*/
   ...
   // pole pocet dnu v mesicich
	...
	// #################  testovaci  ###############################
	//cislo_dne_dnes = 3; // testovaci radek, cislo dne dnes
	//h = 3; // hodina ted
	// #############################################################
	...
	//ShowWindow(hWnd, SW_SHOWMAXIMIZED); // max. okno
    ...
    if (!infile.is_open()) { // nelze otevzit soubor
        ...
        if (outfile.is_open()) {
        	...
        	ShowWindow(hWnd, SW_SHOWNORMAL); // normalni velikost okna (stredni okno)
            //ShowWindow(hWnd, SW_SHOWMAXIMIZED); // maximalizuje okno (minimalizoval ho na radku 37)
        ...
        // soubor historie jiz existuje a nacte znej hodnoty
	...
	*/
	...
	if (hist_usek_roku == 3 && usek_roku == 1){zapis_historii = 1;}
	...
	*/
	...
	if ( zapis_historii == 1 ){
        ...
        if (outfile.is_open()){ // ochrana proti Read-Only
			...
			// ceka 0.5 vteriny ale jen v pripade ze zapisuje ho historie ze nastal novy rok
        	...
        	ShowWindow(hWnd, SW_SHOWNORMAL); // normalni velikost okna (stredni okno)
            //ShowWindow(hWnd, SW_SHOWMAXIMIZED); // maximalizovane okno (minimalizovano na radku 37)
	...
	if ( cas_plus_hodina == 1) {
		ShowWindow(hWnd, SW_SHOWNORMAL); // stredni okno
		cout<<endl;
