...
/* toto je "ukecana" verze programu a vypisuje co nejvice informaci, slouzi spise ktomu aby clovek 
zjistil jak program funguje, pro bezne poziti slouzi verze "ticha verze"
samozdrejme soubor "exe" si lze prejmenovat podle sveho jakoliv
zmena nazvu souboru historie pripadne i doplneni cesty k nemu ja na radku 128
...
int main(int argc, char** argv) {
   ...
   // pole pocet dnu v mesicich
	...
	int offset_rijen = 0, posledni_nedele_v_rijnu = 0, cislo_dne_rijen = 0;
	...
	if (cislo_dne_dnes < cislo_dne_brezen){cout<<"usek 1-A"<<endl; usek_roku = 1;}
	if (cislo_dne_dnes == cislo_dne_brezen && h < 2){cout<<"usek 1-B"<<endl; usek_roku = 1;}
	if (cislo_dne_dnes == cislo_dne_brezen && h >= 2){cout<<"usek 2-A"<<endl; usek_roku = 2;}
	if (cislo_dne_dnes > cislo_dne_brezen && cislo_dne_dnes < cislo_dne_rijen){cout<<"usek 2-B"<<endl;usek_roku = 2;}
    if (cislo_dne_dnes == cislo_dne_rijen && h < 3){cout<<"usek 2-C"<<endl; usek_roku = 2;}
    if (cislo_dne_dnes == cislo_dne_rijen && h >= 3){cout<<"usek 3-A"<<endl; usek_roku = 3;}
    if (cislo_dne_dnes > cislo_dne_rijen){cout<<"usek 3-B"<<endl; usek_roku = 3;}
	
	// testovaci radek, chyba usek_roku zustal v hodnote nula *
	...
	*/
    ...
    if (hist_usek_roku == 1 && usek_roku == 2){cout<<"1-2"<<endl; zapis_historii = 1; cas_plus_hodina = 1;}
	if (hist_usek_roku == 2 && usek_roku == 1){cout<<"2-1"<<endl; zapis_historii = 1; cas_minus_hodina = 1;}
	if (hist_usek_roku == 2 && usek_roku == 3){cout<<"2-3"<<endl; zapis_historii = 1; cas_minus_hodina = 1;}
	...
	*/
	...
	if ( zapis_historii == 1) {
        ...
        if (outfile.is_open()) { // ochrana proti Read-Only
			...
			// ceka 10 vterin ale jen v pripade ze zapisuje ho historie ze nastal novy rok
